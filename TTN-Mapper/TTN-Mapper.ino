...
/* https://github.com/lnlp/LMIC-node/blob/main/src/LMIC-node.cpp */
...
/* die folgenden Keys werden beim Registrieren des TTN-Devices in der TTN-Console erzeugt, deshalb hier nur Platzhalter: */
  ...
  switch (ev) {
    ...
    case EV_TXCOMPLETE:
      ...
      if (LMIC.dataLen != 0 || LMIC.dataBeg != 0) {        
        /* Received data */
        int16_t rssi = LMIC.rssi - RSSI_OFFSET;
        int16_t snrTenfold = (LMIC.snr * 10) / 4;
    ...
    case EV_SCAN_FOUND:
      ...
      u8x8.drawString(0, 7, "EV_SCAN_FOUND   ");
      break;
    case EV_TXSTART:
    ...
    case EV_RXSTART:
      ...
      u8x8.drawString(0, 7, "EV_RXSTART   ");
      break;
...
void do_send(osjob_t* j) {
  ...
  if (LMIC.opmode & OP_TXRXPEND) {
    ...
    if ( ((int)gps.hdop.value() < HDOP_MAX) && ((int)gps.hdop.value() != 0) ) {
      ...
      //build_packet();
      LMIC_setTxData2(1, txBuffer, sizeof(txBuffer), send_confirmed);
      Serial.println();
